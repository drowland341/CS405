...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Detect patterns like "or x=x" where x can be any value
  if (lowered_sql.find(" or ") != std::string::npos &&
      (lowered_sql.find("=1") != std::string::npos || lowered_sql.find("='") != std::string::npos)) 
  {
      std::cout << "[ALERT] Potential SQL injection detected in query: " << sql << std::endl;
      return false; // Prevent execution of the query
  }

  // IF no injection detected, execute the query
