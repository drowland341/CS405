...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 4+: data
    ...
    file << 1900 + ltm.tm_year << "-" << 1 + ltm.tm_mon << "-" << ltm.tm_mday << std::endl;
...
int main()
{
  ...
  const std::string decrypted_file_name = "decrypteddatafile.txt"; // Fixed a typo "decrytpteddatafile -> decrypteddatafile"
  const std::string source_string = read_file(file_name);
  const std::string key = "password";

  // Read input file, error handling for fail to read
